...
let lastFrameTime = 0
let isPaused = false
let routeRouterUnregister: (() => void) | null = null
let routeAfterUnregister: (() => void) | null = null
let resumeNavTimer: ReturnType<typeof setTimeout> | null = null

// Target color for smooth transition
const targetColor = new THREE.Color()
const currentColor = new THREE.Color()

// useRouter 必须在 setup 同步阶段调用，不能放进 onMounted 里
...
const animate = (now?: number) => {
  ...
  // 暂停或不可见时直接跳过这一帧（仍然预约下一帧，等条件恢复时无缝继续）
...
const handleVisibilityChange = () => {
  // 标签页切到后台时，animate 已经会跳过，但 raf 在后台浏览器会自动节流到 1Hz，无需手动处理
  // 这里只用于切回来时立刻渲染一帧，避免回来时第一眼看到旧画面
  if (!document.hidden && material) {
    lastFrameTime = 0
  }
}
...
onMounted(() => {
  ...
  // 路由切换期间暂停渲染，让 chunk 下载 / 新页面挂载独享主线程，
  // 切换完成后再延迟一帧恢复，肉眼不会发现背景"卡"过
  if (router) {
    routeRouterUnregister = router.beforeEach((_to, _from, next) => {
      isPaused = true
      if (resumeNavTimer) clearTimeout(resumeNavTimer)
      next()
    })
    routeAfterUnregister = router.afterEach(() => {
      if (resumeNavTimer) clearTimeout(resumeNavTimer)
      resumeNavTimer = setTimeout(() => {
        isPaused = false
        lastFrameTime = 0
      }, 50)
    })
...
onUnmounted(() => {
  ...
  observer.disconnect()
  if (routeRouterUnregister) routeRouterUnregister()
  if (routeAfterUnregister) routeAfterUnregister()
  if (resumeNavTimer) clearTimeout(resumeNavTimer)
  if (renderer) {
    renderer.dispose()
