    ...
    <div class="container mx-auto px-4 md:px-8 flex justify-between items-center">
      ...
      <div class="hidden md:flex items-center space-x-4">
        ...
        <div v-else class="flex items-center space-x-4">
          <!-- 会员升级按钮 -->
          <button 
            v-if="user?.subscription.tier === 'free'"
            @click="openPricingModal"
            class="px-4 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-full font-bold text-sm shadow-lg hover:from-yellow-600 hover:to-yellow-700 transition-all transform hover:scale-105 flex items-center"
          >
            <i class="fas fa-crown mr-2"></i>
            升级会员
          </button>
          <div 
            v-else-if="user?.subscription.tier === 'pro'"
            class="px-3 py-1 bg-primary/10 text-primary rounded-full font-bold text-xs flex items-center border border-primary/30"
          >
            <i class="fas fa-graduation-cap mr-1"></i>
            居士
          </div>
          <div 
            v-else-if="user?.subscription.tier === 'master'"
            class="px-3 py-1 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 text-yellow-600 rounded-full font-bold text-xs flex items-center border border-yellow-500/30"
          >
    ...
    <div id="mobileMenu" :class="['mobile-menu', { 'hidden': !isMobileMenuOpen }]">
      <div class="container mx-auto px-4 py-3 flex flex-col space-y-3">
        ...
        <div v-if="isLoggedIn" class="flex flex-col space-y-3 pt-2 border-t border-gray-200 mt-2">
          ...
          <button 
            v-if="user?.subscription.tier === 'free'"
            @click="openPricingModal(); closeMobileMenu()"
            class="w-full px-4 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg font-bold text-sm shadow-lg flex items-center justify-center"
          >
            <i class="fas fa-crown mr-2"></i>
            升级会员，解锁更多功能
          </button>
          <div 
            v-else-if="user?.subscription.tier === 'pro'"
            class="w-full px-4 py-2 bg-primary/10 text-primary rounded-lg font-bold text-sm flex items-center justify-center border border-primary/30"
          >
          ...
          <div 
            v-else-if="user?.subscription.tier === 'master'"
            class="w-full px-4 py-2 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 text-yellow-600 rounded-lg font-bold text-sm flex items-center justify-center border border-yellow-500/30"
          >
            <i class="fas fa-crown mr-2"></i>
            当前身份：宗师
          </div>
