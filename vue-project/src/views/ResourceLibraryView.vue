            ...
            >
              <div class="p-6">
                <div class="absolute top-3 right-3 flex items-center gap-2">
                  <span v-if="resource.isPaid && !isResourceUnlocked(resource.id)" class="px-2 py-1 text-xs font-bold text-white bg-yellow-500 rounded">
                    <i class="fas fa-lock mr-1"></i>付费
                  </span>
                  <span v-else-if="resource.isPaid" class="px-2 py-1 text-xs font-bold text-white bg-green-500 rounded">
                    <i class="fas fa-unlock mr-1"></i>已解锁
                  </span>
                  <span :class="['px-2 py-1 text-xs font-bold text-white rounded', getTypeColor(resource.type)]">
                ...
                <button 
                  ...
                  :class="['absolute top-3 left-3 w-8 h-8 rounded-full flex items-center justify-center transition-all', 
                    isFavorited(resource.id) ? 'bg-red-100 text-red-500' : 'bg-gray-100 text-gray-400 hover:text-red-500 hover:bg-red-50']"
                >
                  <i :class="isFavorited(resource.id) ? 'fas fa-heart' : 'far fa-heart'"></i>
                </button>
                ...
                <div class="mt-4 flex gap-2">
                  <button 
                    v-if="resource.isPaid && !isResourceUnlocked(resource.id)"
                    @click.stop="unlockResource(resource)"
                    class="flex-1 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg font-medium hover:from-yellow-600 hover:to-yellow-700 transition-colors text-sm"
                  >
                    <i class="fas fa-unlock mr-1"></i>
                    解锁 ¥{{ resource.price || 9.9 }}
                  </button>
                  <button 
                    v-else
                    @click.stop="downloadResource(resource)"
                    class="flex-1 py-2 bg-primary text-white rounded-lg font-medium hover:bg-primary/90 transition-colors text-sm"
                  >
...
const unlockedResources = ref<number[]>([])

onMounted(() => {
  const savedFavorites = localStorage.getItem('resource_favorites')
  if (savedFavorites) {
    favorites.value = JSON.parse(savedFavorites)
  }
  
  const savedUnlocked = localStorage.getItem('unlocked_resources')
  if (savedUnlocked) {
    unlockedResources.value = JSON.parse(savedUnlocked)
  }
})
...
const toggleFavorite = (id: number) => {
  ...
  localStorage.setItem('resource_favorites', JSON.stringify(favorites.value))
}

const isResourceUnlocked = (id: number): boolean => {
  if (user.value?.subscription.tier === 'master') {
    return true
  }
  return unlockedResources.value.includes(id)
}
...
const unlockResource = (resource: Resource) => {
  ...
  const price = resource.price || 9.9
  ...
  if (confirm(`确定要解锁此资源吗？\n原价：¥${price}\n会员价：¥${finalPrice.toFixed(2)}`)) {
    unlockedResources.value.push(resource.id)
    localStorage.setItem('unlocked_resources', JSON.stringify(unlockedResources.value))
    alert('解锁成功！您现在可以下载此资源。')
  }
}
