        ...
        >
          ...
          <div class="p-6 flex-1 flex flex-col">
            ...
            <div class="mt-auto">
              ...
              <div v-if="canAccess && getCourseProgress(course.id) > 0" class="mb-3">
              ...
              >
                <span v-if="canAccess">
                  <i :class="getCourseProgress(course.id) > 0 ? 'fas fa-play-circle' : 'fas fa-play-circle'" class="mr-2"></i>
...
const loadProgress = () => {
  courses.forEach(course => {
    const key = `course_${course.id}_progress`
    const saved = localStorage.getItem(key)
    if (saved) {
      const completed = JSON.parse(saved)
      courseProgress.value[course.id] = Math.round((completed.length / course.chapterCount) * 100)
    }
  })
}
