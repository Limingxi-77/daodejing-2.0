<template>
  <div data-testid="tts-page" class="tts-container">
    <!-- 头部 -->
    ...
    <main class="tts-main">
      ...
      <aside class="tts-control-panel">
        ...
        <section class="tts-panel-section" v-if="currentMode !== 'custom'">
          <h3>选择章节</h3>
          <select v-model="selectedChapter" data-testid="tts-chapter-select" class="tts-select-box">
      ...
      <section class="tts-content-area">
        ...
        <div class="tts-card">
          <div class="tts-card-header">
            <h3>语音播放</h3>
            <span data-testid="tts-status" :class="['tts-status-badge', audioStatus]">{{ statusText }}</span>
          </div>
          <div class="tts-card-body">
            <!-- 音频可视化 -->
        ...
        <div class="tts-card">
          ...
          <div class="tts-card-body">
            <ul data-testid="tts-history-list" class="tts-history-list">
              <li 
...
const loadTtsTasks = async () => {
  try {
    const response = await fetchTtsTasks()
    ...
    console.warn('TTS 任务历史 API 不可用:', error)
  }
}
