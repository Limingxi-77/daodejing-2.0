<template>
  <div data-testid="community-page" class="pt-24 pb-12 px-4 md:px-8 min-h-screen bg-gray-50">
    <div class="container mx-auto max-w-5xl">
      ...
      <!-- 创建帖子表单 -->
      <section data-testid="community-compose" class="bg-white rounded-lg p-6 mb-8 shadow-sm border border-secondary/20 relative">
        <h2 class="text-xl font-bold text-dark mb-4">分享您的想法</h2>
        <form @submit.prevent="publishPost" data-testid="community-post-form">
          <div class="mb-4">
      ...
      <!-- 标签导航 -->
      <section class="mb-8">
        <div data-testid="community-tabs" class="flex border-b border-gray-200 mb-6">
          <button 
            v-for="tab in tabs" 
            :key="tab.id"
            @click="currentTab = tab.id"
            :data-testid="`community-tab-${tab.id}`"
      ...
      <!-- 帖子列表 -->
      <section>
        <div data-testid="community-post-list" class="space-y-6">
          ...
          <article 
            ...
            :key="post.id"
            data-testid="community-post-item"
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-3">
                    <button
                      ...
                      class="flex items-center text-gray-400 hover:text-red-500 transition-colors text-sm"
                    >
    ...
    <div v-if="showReportModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showReportModal = false">
      <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
        <h3 class="text-xl font-bold text-gray-800 mb-4">举报内容</h3>
        <p class="text-sm text-gray-500 mb-4">请选择举报理由：</p>

        <div class="space-y-2 mb-4">
          <label v-for="reason in reportReasons" :key="reason" class="flex items-center cursor-pointer p-3 rounded-lg border hover:bg-gray-50 transition-colors" :class="{ 'border-primary bg-primary/5': reportReason === reason, 'border-gray-200': reportReason !== reason }">
            <input type="radio" v-model="reportReason" :value="reason" class="hidden">
            <span class="text-sm">{{ reason }}</span>
          </label>
        </div>

        <div class="mb-4">
          <label class="block text-sm text-gray-600 mb-1">补充说明（选填）</label>
          <textarea v-model="reportDetail" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="请提供更多细节，帮助我们更好地处理..."></textarea>
        </div>

        <div class="flex justify-end gap-3">
          <button @click="showReportModal = false" class="px-4 py-2 text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">取消</button>
          <button @click="submitReport" :disabled="!reportReason || isReporting" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 transition-colors">
...
const reportTargetId = ref<string | number | null>(null)
const reportReason = ref('')
const reportDetail = ref('')
const isReporting = ref(false)
const reportReasons = ['违禁内容', '垃圾广告', '人身攻击', '虚假信息', '其他']
...
const mapApiPost = (post: CommunityPost): Post => ({
  id: post.id,
  user: post.authorName || post.username,
  time: post.createdAt ? formatDate(post.createdAt) : '刚刚',
  title: post.title,
...
const toggleLike = async (id: string | number) => {
  ...
  const nextLiked = index === -1
  
  try {
    const result = await setCommunityLike(strId, nextLiked)
    if (index > -1) {
    ...
    if (post) post.likes = result.likeCount
  } catch {
    if (index > -1) {
      likes.value.splice(index, 1)
      if (post) post.likes--
...
const toggleBookmark = async (id: string | number) => {
  ...
  const nextBookmarked = index === -1
  
  try {
    await setCommunityBookmark(strId, nextBookmarked)
  } catch (error) {
    console.warn('收藏同步失败，已使用本地缓存:', error)
  }
...
const openReportModal = (postId: string | number) => {
...
const submitReport = async () => {
  if (!reportReason.value || !reportTargetId.value) return
  isReporting.value = true
  try {
    await reportCommunityContent('post', String(reportTargetId.value), reportReason.value, reportDetail.value)
    showNotification('举报已提交，管理员将进行审核。您可在收件箱中查看处理进度。')
    showReportModal.value = false
  } catch (error) {
    console.warn('举报提交失败:', error)
    showNotification('举报提交失败，请稍后再试')
  } finally {
...
const publishPost = async () => {
  ...
  try {
    ...
    allPosts.value.unshift(mapApiPost(response.post))
    ...
    newPost.value = { title: '', content: '', tags: '' }
    showNotification('后端不可用，已暂存到本地列表', 'info')
  }
}
