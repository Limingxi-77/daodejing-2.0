          ...
          >
            <div class="flex items-start mb-4">
              ...
              <div class="flex-1">
                ...
                <div class="text-gray-700 mb-4 post-content" v-html="sanitizeHtml(post.content)"></div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="relative">
                    ...
                    <div v-if="activeShareMenu === post.id" class="absolute bottom-full right-0 w-40 bg-white rounded-lg shadow-xl py-2 z-10 mb-2">
                      ...
                      <button @click="handleShare('report', post.id)" class="w-full text-left px-4 py-2 hover:bg-gray-50 text-red-500 flex items-center">
...
const isSelf = (user: string) => {
  if (!isLoggedIn.value || !currentUser.value) return false
  return user === currentUser.value.display_name || user === currentUser.value.username
}
const toggleFollow = (user: string) => {
  if (isSelf(user)) {
    showNotification('不能关注自己哦', 'info')
    return
  }
...
const handleShare = (type: string, postId?: string | number) => {
  activeShareMenu.value = null
  if (type === 'copy') {
    navigator.clipboard.writeText(window.location.href)
    showNotification('链接已复制到剪贴板')
  } else if (type === 'report') {
    reportTargetId.value = String(postId || '')
...
const sanitizeHtml = (html: string): string => {
  const allowedTags = ['b', 'i', 'u', 'strong', 'em', 'p', 'br', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li', 'blockquote', 'pre', 'code', 'span', 'div', 'a']
          ...
          if (allowedAttributes[tagName].includes(attr.name)) {
            if (attr.name === 'href' && tagName === 'a') {
              ...
              const safeStyles = style.split(';')
                ...
                .join('; ')
              if (safeStyles) {
                newElement.setAttribute('style', safeStyles)
              }
            } else {
