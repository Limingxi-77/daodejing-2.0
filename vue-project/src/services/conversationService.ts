...
export const createConversation = async (title: string, persona: string = 'scholar'): Promise<Conversation> => {
  try {
    ...
    // 尝试保存到后端
    try {
      const savedConversation = await apiClient<Conversation>('/conversations', {
        method: 'POST',
        body: newConversation
      })
      conversations.value.unshift(savedConversation)
      currentConversation.value = savedConversation
      return normalizeConversation(savedConversation)
    } catch (backendError) {
...
/**
 * 保存消息到当前会话
 * 注意：仅负责持久化（更新元数据 + 后端 POST）。本地 messages 数组的 push 由调用方负责，
 * 避免与 chatStore 的 messages.value.push 重复。
 */
export const saveMessage = async (message: ChatMessage): Promise<void> => {
  ...
  try {
    ...
    // 自动生成更具体的标题（如果会话刚开始）—— 基于当前 messages 内容判断
    const conversationMessages = currentConversation.value.messages
    if (conversationMessages.length === 1 && conversationMessages[0]?.type === 'user') {
      currentConversation.value.title = conversationMessages[0].content.slice(0, 30) + '...'
    }

    // 尝试保存到后端
    try {
      await apiClient(`/conversations/${currentConversation.value.id}/messages`, {
        method: 'POST',
        body: message
      })
    } catch (backendError) {
...
export const loadConversations = async (): Promise<Conversation[]> => {
  try {
    ...
    // 尝试从后端加载
    try {
      const data = await apiClient<Conversation[]>('/conversations')
...
export const searchConversations = async (filter: SearchFilter): Promise<Conversation[]> => {
  try {
    ...
    try {
      ...
      const data = await apiClient<Conversation[]>(`/conversations/search?${queryParams}`)
...
export const deleteConversation = async (conversationId: string): Promise<void> => {
  try {
    // 尝试后端删除
    try {
      await apiClient(`/conversations/${conversationId}`, {
        method: 'DELETE'
      })
    } catch (backendError) {
