...
export class CosyVoiceService {  private defaultVoice: string
  ...
  constructor(config: CosyVoiceConfig) {    this.defaultVoice = config.DEFAULT_VOICE || 'longxiaochun'
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (data.mode === 'browser-fallback') {
          // 由上层（useTTS）决定降级到浏览器原生 TTS，避免在此处阻塞导致按钮"停不下来"
          throw new Error('BROWSER_FALLBACK_REQUESTED')
        }
        if (data.audioUrl) {
          return data.audioUrl
        }
        if (data.raw?.output?.audio) {
          return data.raw.output.audio
        }
